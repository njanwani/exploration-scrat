"""
This file runs the smart explore algorithm. The general layout and explanation
...
grid_array = pickle.load(open(f'{MAP_DIR}/mapsm_09.pkl', 'rb'))
metric = []
...
while True:
    ...
    metric.append(np.mean([int(grid.explore_grid.pose_grid[(x,y)].freespace < FREESPACE_ALPHA or grid.explore_grid.pose_grid[(x,y)].freespace > 1 - FREESPACE_ALPHA) for x,y in grid.pose_grid]))
    print(metric[-1], end='\r')
    i += 1